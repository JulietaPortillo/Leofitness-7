...
class InvoicesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (! $request->has('drp_start') or ! $request->has('drp_end')) {
            $drp_placeholder = 'Selecione filtro de intervalo de fechas';
        } else {
    ...
    public function applyDiscount($id, Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            flash()->success('Descuento fue aplicado correctamente');

            return redirect(action('InvoicesController@show', ['id' => $id]));
        } catch (\Exception $e) {
            DB::rollback();
            flash()->error('Error al aplicar descuento. Por favor intente nuevamente');
